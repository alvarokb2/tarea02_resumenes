...
tablaPtr getTabla(booTokenPtr __access, booTokenPtr __location, booTokenPtr __block) {
    ...
    tablaPtr aux = NULL;
    booTokenPtr access = __access;
    booTokenPtr location;
    booTokenPtr block;
    while (access != NULL) {
        location = __location;
        ...
        // busca geoname_id
        while (block != NULL) {
            if (compararIp(access->value, block->value)) {
                if (block->next == NULL) break;
                geoname_id = block->next->value;
                break;
            } else {
                // avance a la siguiente ip
                block = block->next;
        ...
        if (geoname_id != NULL) {
            while (location != NULL) {
                if (*geoname_id == *location->value) {
                    if (location->next == NULL) break;
                    country_name = location->next->value;
                    break;
                } else {
                    location = location->next;
                    if (location == NULL) location = location->next;
                }
            }
        }
        char * estado = NULL;
